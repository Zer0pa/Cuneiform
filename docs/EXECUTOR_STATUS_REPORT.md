...
artefact on GitHub — but cannot independently re-hash the manifests.

Operational endpoints (pod hostname/port/container id) and monorepo root path
are held separately in Zer0pa's internal operations registry and are **not**
committed to this repo. See the closeout brief for the canonical source.

## Review Fast Path (≤ 15 minutes)

1. `README.md` — current authority block.
2. `SOVEREIGN_PRD.md` — the gate I was working against.
3. `docs/evidence/CUNEIFORM_PHASE2_GATE_STATUS.md` — failed gate statement.
   Confirm it is unchanged.
4. `docs/PATH_REWRITE_LEDGER.md` — the source-to-destination freeze +
   exclusions.
5. `docs/MINIMAL_SMOKE_TARGET.md` — why `SMOKE-01-MANIFEST-VALIDATION` was
   chosen over candidates 1, 2, and 4.
6. `docs/evidence/ARTEFACT_CHECKSUMS.md` — SHA-256 pins.
7. `artefacts/smoke/manifest_validation_report.json` — real smoke run output.
8. `artefacts/smoke/hf_upload_verify.json` — HF post-upload checksum
   verification.
9. `docs/HF_CUSTODY_REGISTER.md` — token-verified HF state.
10. `.gpd/phases/0{1,2,3}-*/0{1,2,3}-01-SUMMARY.md` — per-phase close.
11. `AUDITOR_PLAYBOOK.md` — claim-to-evidence map + replay command.
...
| Item | Basis |
|---|---|
| The 6 manifest SHA-256 pins | Computed on the upstream pod against the files in `<MONOREPO>/workspace/artifacts/cuneiform/` and `<MONOREPO>/workspace/share/science_engineering_review_2026-04-10/`. Stable across both locations; `M-01` and `M-02` are byte-identical as expected. |
...
| HF custody matches the pins | Post-upload verification re-downloaded each file from the recorded revision and re-hashed; 6/6 matched pinned values. Re-verified during closeout — see `docs/HF_CUSTODY_REGISTER.md`. |
...
I consider this PRD + closeout complete. If a reviewer disagrees, the
disagreement should be recordable against a specific file, line, and missing
artefact path. If nothing like that turns up, the lane should sit in
