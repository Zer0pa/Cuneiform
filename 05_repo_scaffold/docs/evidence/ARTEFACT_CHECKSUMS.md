...
> **Phase 02 artefact.** Pinned 2026-04-24 from upstream pod
> `38.80.152.147:34587` paths under `/workspace/ZPE-Cipher/`. These pins are
> append-only. A pin may **never** be silently changed; if a checksum drifts,
> open a blocker and record the divergence as evidence.

## Pinning Procedure

1. Source paths are on the pod (`upstream_path` column).
2. SHA-256 computed via `sha256sum` on the pod.
3. Files are not vendored into git; they are routed to HF dataset
   `Zer0pa/cuneiform-control-artefacts` (private) and re-verified post-upload.
